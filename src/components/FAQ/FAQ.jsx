...
export default function FAQ({ min = false }) {
  ...
  const groupedByType = data.reduce((acc, doc) => {
    if (!acc[doc.type]) {
      acc[doc.type] = []
    }
  ...
  const iniciar = groupedByType['iniciar'] || []
  const general = groupedByType['general'] || []
  const donacion = groupedByType['donacion'] || []
  const pago = groupedByType['pago'] || []
  const seguridad = groupedByType['seguridad'] || []
  ...
  if (error) {
    return (
      <h1 className={styles.error}>
        Error: {error} - Estamos trabajando en solucionarlo.
      </h1>
    )
  }
